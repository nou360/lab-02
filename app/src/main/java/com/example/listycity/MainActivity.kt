...
fun CityListScreen(
    ...
    Column(modifier = modifier.fillMaxSize()) {
        ...
        LazyColumn(modifier = modifier.fillMaxSize()) {
            items(cities) { city ->
                val Select = (city==selectCity)
                CityRow(city = city, Selected = Select, onSelect = {selectCity = if (Select) null else city})
            }
        }
    }
}
@Composable
fun CityRow(city:String,Selected: Boolean,onSelect: () -> Unit){
    Text(
        text = city,
        fontSize = 28.sp,
